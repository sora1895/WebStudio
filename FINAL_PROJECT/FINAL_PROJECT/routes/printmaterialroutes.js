...
exports.GetPMaterial = function (req, res) {
  ...
  exports.GetMaterialByName = function (req, res) {
    ...
    console.log(name);
    connection.query('SELECT * FROM printingmaterial WHERE Material_Name=?',[name], function (error, results, fields) {
      if (error) {
        res.send({
          "code": 400,
          "failed": "error ocurred"
  ...
  exports.GetMaterialIdByName = function (req, res) {
    ...
    console.log(name);;
    connection.query('SELECT Material_ID FROM printingmaterial WHERE Material_Name=?',[name], function (error, results, fields) {
      if (error) {
        res.send({
          "code": 400,
          "failed": "error ocurred"
  ...
  exports.AddMaterial = function (req, res) {
    ...
    connection.query("INSERT INTO printingmaterial (Material_Name,Material_Detail,Material_Price) VALUES ?",[a], function (error, results) {
      //console.log(values);
      if (error) {
        res.send({
          "code": 400,
  ...
  exports.EditMaterial = function (req, res) {
    ...
    connection.query('UPDATE printingmaterial SET Material_name=?,Material_detail=?,Material_price=? where Material_id=?', [newName,newDetail,newPrice,id], function (error, results, fields) {
      if (error) {
        res.send({
          "code": 400,
          "failed": "error ocurred"
